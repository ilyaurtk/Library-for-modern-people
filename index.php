	...
	foreach ($rows_job as $row)//цикл в котором за один проход записываются в переменную row данные ОДНОЙ строки в таблице по порядку
		{
			...
			echo "<td><a href='user_update.php?ID=".$id."'>Редактировать</a></td><td><a href='user_delete.php?ID=".$id."'>Удалить</a></td>";//перенаправление на страницу user_delete.php с передачей id строки
			echo '</tr>';
    	}
    	echo "</table>";
	}
...
<?php header('Location: login.php');//перенаправление на страницу авторизации?>
<?php endif; ?>
</body>
</html>